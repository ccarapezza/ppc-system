...
bool AlarmsManager::loadFromStorage() {
    ...
    // Note: reassignRelayFunctions(relay) must be called after loading
    // with the appropriate RelayPort instance
...
void AlarmsManager::reassignRelayFunctions(RelayPort& relay) {
    Alarm *currentAlarm = firstAlarm;

    while (currentAlarm != nullptr) {
        if (currentAlarm->execute == nullptr && !currentAlarm->extraParams.empty()) {
            int channel = currentAlarm->extraParams[0]; // 1-based channel

            if (channel >= 1 && channel <= relay.channelCount()) {
                if (currentAlarm->extraParams.size() > 1) {
                    int state = currentAlarm->extraParams[1];
                    int zeroBasedChannel = channel - 1;

                    if (state == 1) {
                        currentAlarm->execute = [&relay, zeroBasedChannel](int) {
                            relay.setState(zeroBasedChannel, true);
                        };
                        Serial.println("Reassigned ON function to alarm: " + currentAlarm->name);
                    } else if (state == 0) {
                        currentAlarm->execute = [&relay, zeroBasedChannel](int) {
                            relay.setState(zeroBasedChannel, false);
                        };
