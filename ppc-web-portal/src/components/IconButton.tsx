...
function IconButton({ icon, outlined = false, indicator, className, ...props }: IconButtonProps) {
    ...
    return (
        <button type="button" className={`relative ${outlined ? outlinedClass : containedClass} ${className}`} {...props}>
            ...
            {indicator&&
                <div className={`absolute inline-flex items-center justify-center px-1 w-auto min-w-5 h-auto text-xs font-bold text-white bg-red-500 rounded-full -top-2 -end-2 dark:border-gray-900`}>
                    {indicator}
                </div>
            }
        </button>
